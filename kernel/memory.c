...
/* 安装1页大小的 vaddr， 专门针对 fork 时虚拟地址位图无需操作的情况 */
...
/* 返回 arena 中第 idx 个内存块 */
/* 在 arena 指针指向的页框中，跳过元信息部分，即 struct arena 的大小，再用 idx 乘以该 arena
    *  中的内存块大小，最终的地址便是 arena 中第 idx 个内存块的首地址 
    * */
...
void *sys_malloc(uint32_t size)
{
    ...
    if (size > 1024)
    {
        ...
        if (a != NULL)
        {
            memset(a, 0, page_cnt * PG_SIZE); //将分配内存清0
        }
        ...
        lock_release(&mem_pool->lock);
        return (void *)(a + 1); //跨过arena 大小，把剩下内存返回
    }
    else
    {
        // 若申请的内存小于等于 1024，可在各种规格的 mem_lock_desc 中取匹配
        uint8_t desc_idx;
        /* 从内存块描述符中匹配合适的内存块规格 */
        ...
        /* 若 mem_block_desc 的 free_list 中已经没有可用的 mem_lock，就创建新的 arena 提供 mem_block */
        if (list_empty(&descs[desc_idx].free_list))
        {
            a = malloc_page(PF, 1); //分配一页作为 arena
            if (a == NULL)
            {
                lock_release(&mem_pool->lock);
                return NULL;
            ...
            /* 对于分配的小块内存，将 desc 置为相应内存块描述符 , cnt 置为此 arena 可用的内存块数量，large置为false */
        ...
        a = block2arena(b); //获取内存块 b 所在的arena
        a->cnt--;           //将此arena 中的空闲内存块数量减1
...
void mem_init()
{
    ...
    block_desc_init(k_block_descs); // 初始化内核的 mem_block_desc 数组 descs，为 malloc 做准备
